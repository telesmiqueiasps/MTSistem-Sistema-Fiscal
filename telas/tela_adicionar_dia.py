...
class TelaAdicionarDia:
    ...
    def criar_interface(self):
        ...
        tk.Label(
            content,
            text="Adicionar Dia de Produção",
        ...
        tk.Label(
            total_frame,
            text="Total de Sacos Produzidos no Dia:",
            font=('Segoe UI', 10),
            bg=CORES['bg_card'],
            fg=CORES['text_dark']
        ).pack(side="left", padx=(0, 10))
        
        self.entry_total_sacos = tk.Entry(
            total_frame,
            font=('Segoe UI', 11),
            relief='solid',
            bd=1,
        ...
        tk.Label(
            divisao_frame,
            text="Houve divisão de produção?",
            font=('Segoe UI', 10),
            bg=CORES['bg_card'],
            fg=CORES['text_dark']
        ...
        tk.Radiobutton(
            divisao_frame,
            text="Não - Todos produziram junto",
            variable=self.var_tem_divisao,
            value=False,
            font=('Segoe UI', 10),
            bg=CORES['bg_card'],
            command=self.toggle_divisao
        ).pack(side="left", padx=5)
        
        tk.Radiobutton(
            divisao_frame,
            text="Sim - Houve divisões",
            variable=self.var_tem_divisao,
            value=True,
            font=('Segoe UI', 10),
            bg=CORES['bg_card'],
    ...
    def criar_frame_producao_unica(self):
        tk.Label(
            self.frame_unica,
            text="Buscar e adicionar participantes:",
        ...
        entry_busca = tk.Entry(
            busca_frame,
            textvariable=self.var_busca_unica,
            font=('Segoe UI', 10),
            relief='solid',
            bd=1
        )
        ...
        self.lista_busca_unica = tk.Listbox(
            lista_frame,
            height=6,
            font=('Segoe UI', 9),
        ...
        tk.Button(
            ...
            padx=15,
            pady=8,
            command=self.adicionar_participante_unica
        ).pack(pady=(10, 5))
        ...
        self.participantes_container_unica = tk.Frame(
            self.frame_unica,
            bg='white',
            relief='solid',
            bd=1
        ...
        # Label vazio inicial
        self.lbl_vazio_unica = tk.Label(
            self.participantes_container_unica,
    ...
    def adicionar_participante_unica(self):
        ...
        # Encontra o diarista selecionado
        ...
        def remover():
            ...
            # Se não houver mais participantes, mostra label vazio
        ...
        # Atualiza lista removendo o item adicionado
        self.var_busca_unica.set(self.var_busca_unica.get())
    
    def criar_frame_divisoes(self):
        tk.Label(
            self.frame_divisoes,
            text="Configure as divisões de produção:",
            font=('Segoe UI', 11, 'bold'),
            bg=CORES['bg_card'],
            fg=CORES['primary']
        ).pack(anchor="w", pady=(10, 10))
        
        # Container de divisões
        ...
        tk.Button(
            ...
            padx=15,
            pady=8,
            command=self.adicionar_divisao
        ).pack(pady=(10, 0))
    
    def carregar_diaristas(self):
        # O DAO retorna tuplas: (id, nome, cpf)
        self.diaristas = self.diarista_dao.listar()
    ...
    def adicionar_divisao(self):
        ...
        divisao_frame = tk.Frame(
            self.divisoes_container,
            bg=CORES['bg_main'],
            relief='solid',
            bd=1
        )
        ...
        # Botão remover
        if divisao_id > 0:  # Não pode remover a primeira
            tk.Button(
                header,
                text="🗑️ Remover",
                font=('Segoe UI', 9),
        ...
        tk.Label(
            qtd_frame,
            text="Quantidade de sacos desta divisão:",
            font=('Segoe UI', 9),
            bg=CORES['bg_main']
        ).pack(side="left", padx=(0, 10))
        ...
        tk.Label(
            inner,
            text="(Os sacos serão divididos igualmente entre os selecionados)",
        ...
        # Lista de resultados
        lista = tk.Listbox(inner, height=5, font=('Segoe UI', 9), relief='solid', bd=1)
        lista.pack(fill="x", pady=(5, 5))
        
        # Container dos participantes adicionados
        participantes_container = tk.Frame(inner, bg='white', relief='solid', bd=1)
    ...
    def renumerar_divisoes(self):
        for i, divisao in enumerate(self.divisoes):
            # Atualiza o número da divisão no label
    ...
    def salvar(self):
        ...
        if not self.var_tem_divisao.get():
            ...
            # Divide igualmente
            sacos_por_pessoa = total_sacos // len(selecionados)
            resto = total_sacos % len(selecionados)
            
            for i, d_id in enumerate(selecionados):
                qtd = sacos_por_pessoa + (1 if i < resto else 0)
            ...
            for divisao in self.divisoes:
                try:
                    ...
                    messagebox.showwarning("Atenção", "Informe quantidades válidas para todas as divisões!")
                    return
                ...
                if not selecionados:
                    messagebox.showwarning("Atenção", "Cada divisão deve ter pelo menos um participante!")
                    return
                
                # Divide igualmente os sacos entre os participantes
                sacos_por_pessoa = qtd_sacos // len(selecionados)
                resto = qtd_sacos % len(selecionados)
                
                participantes = []
                for i, d_id in enumerate(selecionados):
                    qtd_part = sacos_por_pessoa + (1 if i < resto else 0)
                    participantes.append({
                        'diarista_id': d_id,
                        'quantidade_sacos': qtd_part
                    })
            ...
            if total_divisoes != total_sacos:
                messagebox.showwarning(
                    "Atenção",
                    f"A soma das divisões ({total_divisoes}) não corresponde ao total de sacos ({total_sacos})!"
                )
                return
        
        # Salva
        if self.dao.adicionar_dia_producao(self.producao_id, data_producao, total_sacos, divisoes_data):
